...
import com.example.noci.databinding.NotesListBinding
...
class NotesAdapter(private val adapterInfo: NotesAdapterInfo, private val adapterDelete: NotesAdapterDelete) :
    ...
    class ViewHolder(val binding: NotesListBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(currentItem: Note, adapterInfo: NotesAdapterInfo, adapterDelete: NotesAdapterDelete) {
            ...
            when (currentItem.type) {
                0 -> {
                    binding.noteType.setImageResource(R.drawable.note_type_health)
                }
                1 -> {
                    binding.noteType.setImageResource(R.drawable.note_type_relationship)
                }
                2 -> {
                    binding.noteType.setImageResource(R.drawable.note_type_career)
                }
                3 -> {
                    binding.noteType.setImageResource(R.drawable.note_type_personal)
                }
                4 -> {
                    binding.noteType.setImageResource(R.drawable.note_type_social)
                }
                5 -> {
                    binding.noteType.setImageResource(R.drawable.note_type_recreation)
                }
            }
        ...
        companion object {
            fun from(parent: ViewGroup): ViewHolder {
                ...
                val binding = NotesListBinding.inflate(layoutInflater, parent, false)
